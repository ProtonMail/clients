...
async fn test_update_message() {
    ...
    tether
        .write_tx::<_, _, StashError>(async |tx| test_starred_label().save(tx).await)
        .await
        .unwrap();
...
async fn test_delete_local_message() {
        ...
        .unwrap();
    {
        ...
        conn.write_tx::<_, _, AppError>(async |tx| Message::mark_deleted(vec![local_id], tx).await)
            .await
            .unwrap();
...

    {
        ...
        conn.write_tx::<_, _, AppError>(async |tx| Message::mark_deleted(ids, tx).await)
            .await
            .unwrap();
...
async fn messages_mark_read() {
    ...
    let check_counters = |mail_stash: Stash<UserDb>,
    ...
    let stash_fun = || mail_stash.clone();
...
async fn messages_mark_unread() {
    ...
    check_counters(mail_stash.clone(), 3, 1).await;
    conn.write_tx::<_, _, StashError>(async |tx| {
    ...
    check_counters(mail_stash.clone(), 2, 0).await;
    conn.write_tx::<_, _, StashError>(async |tx| {
    ...
    .await
    .unwrap();
    check_counters(mail_stash.clone(), 1, 0).await;
    conn.write_tx::<_, _, StashError>(async |tx| {
    ...
    // All conversation messages on label_1 have been marked as read, we should now see an updated
    // conversation count.
    check_counters(mail_stash.clone(), 0, 0).await;
...
async fn label_messages() {
    ...
    let check_full_conversations = |mail_stash: &Stash<UserDb>| -> BoxFuture<'_, ()> {
        let state = &state;
    ...
    check_full_conversations(&mail_stash).await;
...
async fn test_create_message_dependencies(tether: &mut Tether) -> LocalConversationId {
    ...
    tether
        .write_tx::<_, _, StashError>(async |tx| conversation.save(tx).await)
        .await
        .unwrap();
...
async fn watch_messages_in_label() {
    ...
    let handle = Message::watch(&mail_stash).await.unwrap();
...
async fn test_deleting_address_will_trigger_message_deletion() {
    ...
    tether
        .write_tx::<_, _, StashError>(async |tx| Ok(Address::delete_all(tx).await?))
        .await
        .unwrap();
    let addresses = Address::all(&tether).await.unwrap();
        ...
        body: ApiMessageBody {
            attachments: vec![
                ...
                ApiMessageAttachment {
                    ...
                    disposition: mail_api::services::proton::prelude::Disposition::Inline,
                    enc_signature: None,
                    headers: ApiMessageAttachmentHeaders {
...
async fn message_save_preserves_pgp_attachments() {
    ...
    tether
        .write_tx::<_, _, StashError>(async |tx| message.save(tx).await)
        .await
        .unwrap();
    ...
    tether
        .write_tx::<_, _, StashError>(async |tx| message_body.save(tx).await)
        .await
        .unwrap();
    ...
    tether
        .write_tx::<_, _, StashError>(async |tx| msg_body_metadata.save(tx).await)
        .await
        .unwrap();
