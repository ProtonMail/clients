...
const TEST_GO_MIME: &str = r#"Content-Type: multipart/signed; protocol="application/pgp-signature"; micalg=pgp-sha256; boundary="---------------------4a9ea9f4dad3f36079bdb3f1e7b75bd0"; charset=UTF-8
...
const TEST_PK: &str = "-----BEGIN PGP PUBLIC KEY BLOCK-----
...
#[test]
fn test_mime_process_message_verify_signature() {
    let provider = new_pgp_provider();
    let priv_key = provider
        .private_key_import(TEST_PRIV_KEY, "test", DataEncoding::Armor)
        .unwrap();
    let pk = provider
        .public_key_import(TEST_PK, DataEncoding::Armor)
        .unwrap();
    let mime_body = provider
        .new_decryptor()
...
fn test_mime_process_message() {
    ...
    for (attachment, expected_id) in processed_message
        .attachments
        .iter()
        .zip(expected_attachment_id_contains)
    {
        assert!(attachment.id.contains(expected_id));
        assert!(attachment.id.contains("PGPAttachment"));
