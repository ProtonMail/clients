...
./rust-build/build_ios_framework_uniffi.sh proton-mail-uniffi ./mail/mail-uniffi/uniffi.toml "./tmp/ios-framework-debug" mail-ios-debug
...
- `profile` - Cargo profile (optional, defaults to `mail-ios` for release)

### Debugging Setup

To debug Rust code in iOS apps:

1. Build debug framework using `mail-ios-debug` profile (as shown above)
