...
impl EventPoller {
    ...
    pub async fn initialize<Ctx>(
        &self,
        ctx: &Ctx,
        store: &dyn EventStore<Ctx>,
        provider: &dyn EventProvider<Ctx>,
        ...
        Ctx: Send + Sync + 'static,
    {
            ...
            .map_err(EventLoopError::Store)?
        {
            ...
            let event_id = provider.get_latest_event_id(ctx).await?;
    ...
    pub(crate) async fn poll<Ctx>(
        &self,
        ctx: &Ctx,
        store: &dyn EventStore<Ctx>,
        provider: &dyn EventProvider<Ctx>,
        subscribers: &dyn SubscriberList<Ctx>,
        max_events: usize,
    ) -> Result<(), EventLoopError>
    where
        Ctx: Send + Sync + 'static,
    {
        ...
        while processed_event_count < max_events || has_more {
            let Some(raw_event) = self
                .fetch_event(ctx, provider, &previous_event_id)
                .await
                .inspect_err(|e| {
            ...
            if raw_event.is_refresh() {
                subscribers
                    .on_refresh(ctx, raw_event.refresh_flag())
                    .await?;
            } else {
                subscribers.on_event(ctx, &raw_event).await?;
            }

            if let Err(e) = store
                .store(ctx, new_event_id.clone())
                .await
    ...
    where
        Ctx: Send + Sync + 'static,
    {
        let event = provider.get_event(ctx, last_event_id).await?;
...
mod tests {
    ...
    async fn event_collection() {
        ...
        provider
            ...
            .once()
            .in_sequence(&mut sequence)
            .with(predicate::always(), predicate::eq(event_id_1.clone()))
        ...
        subscriber
            ...
            .with(
                predicate::always(),
                predicate::function(move |v: &RawEvent| v.meta.event_id == id),
            )
            .returning(|_, _| Ok(()));
        store
            .expect_store()
            .once()
            .in_sequence(&mut sequence)
            .with(predicate::always(), predicate::eq(event_id_2.clone()))
            .returning(|_, _| Ok(()));
        provider
            .expect_get_event()
            .once()
        ...
        store
            ...
            .once()
            .in_sequence(&mut sequence)
            .with(predicate::always(), predicate::eq(event_id_3.clone()))
            .returning(|_, _| Ok(()));
        ...
        store
            ...
            .once()
            .in_sequence(&mut sequence)
            .with(predicate::always(), predicate::eq(event_id_4.clone()))
            .returning(|_, _| Ok(()));
        ...
        provider
            ...
            .once()
            .in_sequence(&mut sequence)
            .with(predicate::always(), predicate::eq(event_id_4))
            .returning(move |_, _| Ok(event.clone()));
        ...
        let subscribers: Box<dyn SubscriberList<TestCtx>> = Box::new(subscriber);
        evt_poll
            .poll(&ctx, &store, &provider, subscribers.as_ref(), 10)
            .await
            .unwrap();
    }
    ...
    async fn events_that_exceed_limit_with_has_more_are_still_collected() {
        ...
        let ctx = TestCtx;
        evt_poll
            .poll(&ctx, &store, &provider, subscribers.as_ref(), 1)
            .await
            .unwrap();
    }
    ...
    async fn init_fetches_event_id_if_it_does_not_exist() {
        ...
        let ctx = TestCtx;
        evt_poll.initialize(&ctx, &store, &provider).await.unwrap();
        evt_poll.initialize(&ctx, &store, &provider).await.unwrap();
    }
}
