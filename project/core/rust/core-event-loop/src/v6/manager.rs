...
/// all together with [`initialize_all()`](EventManager::initialize_all) or individually with
/// [`initialize()`](EventManager::initialize).
///
/// # Polling
///
/// To run one iteration of event polling, call the [`poll()`](EventManager::poll) method, this will call one
...
/// After 3 attempts, the process is interrupted and the error bubbles up to the
/// [`poll()`](EventManager::poll) site.
///
/// It's up to integrators to decide how to best handle this failure case.
///
/// [`EventSources`]: crate::v6::source::EventSource
///

#[derive(Default)]
pub struct EventManager<Ctx>
where
    Ctx: Send + Sync + 'static,
{
    sources: HashMap<TypeId, EventSourceData<Ctx>>,
    exec_order: Vec<TypeId>,
}

impl<Ctx> EventManager<Ctx>
where
    Ctx: Send + Sync + 'static,
{
    #[must_use]
    ...
    pub fn add<E: EventSource>(
        &mut self,
        provider: Box<dyn EventProvider<Ctx>>,
        store: Box<dyn EventStore<Ctx>>,
        ...
        match self.sources.entry(TypeId::of::<E>()) {
            ...
            Entry::Vacant(v) => {
                ...
                v.insert(EventSourceData::<Ctx>::new::<E>(provider, store));
            }
        }
    ...
    pub fn subscribe<E: EventSource>(
        &mut self,
        subscriber: impl EventSubscriber<Ctx, E> + 'static,
    ) -> Option<EventSubscriberId<E>> {
        let source = self.sources.get_mut(&TypeId::of::<E>())?;
        let type_list =
            <dyn Any>::downcast_mut::<TypedSubscriberList<Ctx, E>>(source.subscribers.as_mut())?;
    ...
    #[instrument(skip_all)]
    pub async fn initialize_all(&mut self, ctx: &Ctx) -> Result<(), EventLoopError> {
        for id in &self.exec_order {
            if let Some(source) = self.sources.get_mut(id) {
                let name = source.name;
                EventPoller::new()
                    .initialize(ctx, source.store.as_ref(), source.provider.as_ref())
    ...
    #[instrument(skip_all)]
    pub async fn initialize<E: EventSource>(&mut self, ctx: &Ctx) -> Result<(), EventLoopError> {
    ...
    #[instrument(skip_all)]
    pub async fn poll(&mut self, ctx: &Ctx) -> Result<(), EventLoopError> {
    ...
    #[instrument(skip_all)]
    pub async fn refresh(&mut self, ctx: &Ctx) -> Result<(), EventLoopError> {
        if self.exec_order.is_empty() {
            tracing::warn!("No event sources registered");
            return Ok(());
        }
        ...
        for id in &self.exec_order {
            self.refresh_source(ctx, *id).await?;
        }

        Ok(())
    }

    async fn poll_source(&self, ctx: &Ctx, type_id: TypeId) -> Result<(), EventLoopError> {
    ...
    async fn refresh_source(&self, ctx: &Ctx, type_id: TypeId) -> Result<(), EventLoopError> {
        let Some(source) = self.sources.get(&type_id) else {
            return Ok(());
        };
        source
            .subscribers
            .on_refresh(ctx, RefreshFlag::from(true))
            .instrument(tracing::debug_span!("Refreshing", name=?source.name))
            .await
    }
}

struct EventSourceData<Ctx>
where
    Ctx: Send + Sync + 'static,
{
    name: &'static str,
    provider: Box<dyn EventProvider<Ctx>>,
    store: Box<dyn EventStore<Ctx>>,
    subscribers: Box<dyn SubscriberList<Ctx>>,
    dependencies: Vec<TypeId>,
}

impl<Ctx> EventSourceData<Ctx>
where
    Ctx: Send + Sync + 'static,
{
    fn new<E: EventSource>(
        provider: Box<dyn EventProvider<Ctx>>,
        store: Box<dyn EventStore<Ctx>>,
    ) -> Self {
        Self {
            ...
            provider,
            store,
            subscribers: TypedSubscriberList::<Ctx, E>::default().boxed(),
...
mod tests {
    ...
    async fn poll_order() {
        ...
        let mut mk_sequence = |id: EventId| -> (
            Box<dyn EventProvider<TestCtx>>,
            Box<dyn EventStore<TestCtx>>,
    ...
    async fn cyclic_dependencies() {
        let mut manager = EventManager::<TestCtx>::new();
    ...
    async fn subscriber_retried_3_on_event_failure() {
        ...
        let mut subscriber_success = MockEventSubscriber::<TestCtx, TestEventSourceA>::new();
        let mut subscriber_failure = MockEventSubscriber::<TestCtx, TestEventSourceA>::new();
        ...
        manager.poll(&ctx).await.unwrap_err();
    }
    ...
    async fn duplicate_subscriber_or_event_source() {
        ...
        let mut subscriber = MockEventSubscriber::<TestCtx, TestEventSourceA>::new();
    ...
    fn make_success_sequence_with_refresh(
        ...
        store
            ...
            .returning(move |_| Ok(Some(event_id.clone())));
        provider
            .expect_get_event()
            .once()
            .in_sequence(sequence)
